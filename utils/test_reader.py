...
def check_point_in_multi_line_string(row,point,multi_lines):
    point_list = []
    for key,val in multi_lines.iteritems():
        x = list(val.geoms)
        for line in x:
        #if line.contains(point):
            if line.distance(point) < 1e-8:
...
for file in os.listdir('../initiatives'):
    if not file.startswith('.'):
        ...
        for line in input_file.geometry:
        ...
        # print(object_type)
            ...
            elif object_type == 'MultiLineString':
                multi_lines = line.geometry

                for row in reader:
                    point = extract_point(row) #creates shapely Point object
                    check_point_in_multi_line_string(row,point,multi_lines) #checks to see if point in polygon and writes 1 to column if True, 0 if false
